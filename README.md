...
### Final Model
$$ \text{LifeExpectancy} = \beta_0 + \beta_1(\text{AdultMortality}) + \beta_2(\text{HepB}^3) + \beta_3(\text{Polio}^3) + \beta_4(\text{Income}) + \beta_5(\log(\text{HIV})) + \beta_6(\log(\text{GDP})) + \beta_7(\log(\text{Thin5}\-\text{9})) + \epsilon $$
